...
class GPVer:
    ...
    def __init__(self, svn_path, ver_type):
        ...
        self.gpver_input_dict = {
            'svn_path':svn_path,
            'ver_typ':[int(x) for x in ver_type],  # 将ver_type的5位字符转换为整数后放到一个列表中
            }
    ...
    def get_ver(self):
        ...
        ver = [0, 0]
        for i in list_file:
            li_i = [int(x) for x in i.split('.')[0:2]]
            if li_i > ver:
                ver = li_i
        ver[1] = ver[1] + 1
        # 获取到两位的新的版本号
        ...
        if self.gpver_input_dict['ver_typ'][4] == 1:
            ...
            ver = [0, 0, 0, 0]
            for i in list_file:
                list_i = [int(x) for x in i.split('.')]
                if list_i > ver:
                    ver = list_i
            ver[1] = ver[1] + 1
            self.gpver_output_dict['pvdb_ver'] = 'Build' + str(ver[0]).zfill(2) + \
            '.' + str(ver[1]).zfill(3) + '.' + str(ver[2]) + '.' + str(ver[3])
